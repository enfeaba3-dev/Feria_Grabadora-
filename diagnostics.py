    ...
    try:
        ...
        return [
            _check("Sistema de audio", bool(inputs), detail, "error"),
            {
                ...
                "ok": bool(inputs),
                "detail": inputs,
                "level": "ok" if inputs else "error",
            },
        ]
    except Exception as exc:
        return [
            _check("Sistema de audio", False, f"{type(exc).__name__}: {exc}", "error")
        ]
...
def _cuda_check() -> dict[str, Any]:
    try:
        ...
        return _check(
            "CUDA para CTranslate2",
            count > 0,
            f"{count} GPU compatible(s)" if count else "No disponible; se usará CPU",
            "warning",
        )
    except Exception as exc:
        return _check(
            "CUDA para CTranslate2", False, f"No disponible: {exc}", "warning"
        )


def _cublas_check() -> dict[str, Any]:
    """CTranslate2 (CUDA 12 build) necesita cublas64_12.dll. Si el
    sistema solo tiene CUDA 13, los paquetes pip nvidia-cublas-cu12
    incluidos en requirements.txt lo aportan dentro del venv."""
    from pathlib import Path

    sp = Path(__file__).resolve().parent / ".venv" / "Lib" / "site-packages" / "nvidia"
    candidates = [
        sp / "cublas" / "bin" / "cublas64_12.dll",
        sp / "cublas" / "bin" / "cublasLt64_12.dll",
        sp / "cuda_nvrtc" / "bin" / "nvrtc64_120_0.dll",
    ]
    found = [str(p) for p in candidates if p.exists()]
    if found:
        return _check(
            "DLLs de CUDA 12 para CTranslate2",
            True,
            f"{len(found)}/{len(candidates)} DLLs presentes en el venv",
            "ok",
        )
    # Comprobar si hay CUDA 13 toolkit (que NO sirve para CTranslate2 4.x)
    cuda13 = (
        list(
            Path("C:/Program Files/NVIDIA GPU Computing Toolkit").glob(
                "CUDA/v13*/bin/cublas64_13.dll"
            )
        )
        if Path("C:/Program Files/NVIDIA GPU Computing Toolkit").exists()
        else []
    )
    if cuda13:
        return _check(
            "DLLs de CUDA 12 para CTranslate2",
            False,
            "Detectado CUDA 13 pero CTranslate2 necesita CUDA 12. "
            "Ejecuta: pip install nvidia-cublas-cu12 nvidia-cuda-nvrtc-cu12",
            "warning",
        )
    return _check(
        "DLLs de CUDA 12 para CTranslate2",
        True,
        "No se encontraron (CPU-only o CUDA ya en PATH)",
        "ok",
    )
...
def _nvidia_check() -> dict[str, Any]:
    ...
    if not executable:
        return _check(
            "Controlador NVIDIA", False, "nvidia-smi no está disponible", "warning"
        )
    try:
        result = subprocess.run(
            [
...
def run_diagnostics(include_audio: bool = True) -> dict[str, Any]:
    ...
    checks.append(
        _check(
            "Python compatible",
            supported,
            f"{platform.python_version()} · recomendado 3.10-3.12",
            "error",
        )
    )
    checks.append(
    ...
    for folder, name in (
        (APP_DIR, "Carpeta de aplicación"),
        (LOG_DIR, "Carpeta de logs"),
        (RUNTIME_DIR, "Carpeta runtime"),
    ):
        try:
    ...
    try:
        ...
        checks.append(
            _check(
                "Espacio libre",
                free_gb >= 8,
                f"{free_gb} GB libres; large-v3-turbo puede requerir varios GB",
                "warning",
            )
        )
    except Exception as exc:
    ...
    warnings = sum(
        1 for item in checks if not item["ok"] and item["level"] == "warning"
    )
    report = {
    ...
    LOGGER.info(
        "Diagnóstico completado | errors=%s | warnings=%s | checks=%s",
        errors,
        warnings,
        len(checks),
    )
...
def create_support_bundle(destination: Path | None = None) -> Path:
    ...
    if destination is None:
        destination = (
            RUNTIME_DIR
            / f"Feria-Transcriber-Soporte-{time.strftime('%Y%m%d-%H%M%S')}.zip"
        )
    report = run_diagnostics(include_audio=True)
    config = load_config()
    with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as archive:
        archive.writestr(
            "diagnostico.json", json.dumps(report, ensure_ascii=False, indent=2)
        )
        archive.writestr(
